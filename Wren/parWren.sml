...
datatype astdecl = DECL of (asttype * string list)
...
datatype astdecls = DECLLST of astdecl list
datatype astcmds =  CMDLST of astcmd list
datatype ast =  PROG of (astdecls * astcmds)
...
fun parse_fact (nil) = (print "Expect Factor!\n"; raise factErr)
  ...
  case lst of 
       IF :: tr => let val (exp, THEN::tc) = parse_exp(tr);
                   ...
                   in (print "Here in if\n";
                       case tc1 of
                           ELSE::ts => (print "Here in else\n"; let val(cmd2,tr1) = parse_cmds(ts)
                                        in case tr1 of
                                        ENDIF::EOS::tr2 => (print "else out!\n";(ASTIF1(exp,cmd,cmd2),tr2) )
                                       | ENDIF::tr2 => (print ("Expect SEMICOLON!\n");raise(ifErr))
                                       | _ => (print ("Expect ENDIF and SEMICOLON!\n");raise(ifErr))
                                        end
                                        )
                           | ENDIF::EOS::tc2 => (print "no else out!\n"; (ASTIF2(exp,cmd),tc2))
  ...
  case lst of
       VAR::tr =>
                 ...
                 in case ts of
                         [END] => PROG(DECLLST(decls),CMDLST(cmds)) 
                       | _ => (print "NOT FINISH THE PROGRAM!\n"; raise blkErr)
                 end
       | BEGIN::tr => (print "NO Declaration Section!\n"; raise blkErr)
       | [END] =>  (print "NO BODY!\n"; raise blkErr)
       | _  => (print "Expect VAR or BEGIN!\n";raise blkErr)
  ...
  and parse_program (nil) =(print " Error in Program\n"; raise progErr)
    | parse_program (lst) =
  case lst of 
       PROGRAM::ID ide :: IS :: tr => parse_block(tr)
